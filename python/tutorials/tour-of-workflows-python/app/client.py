...
async def submit_workflow(
    id: str, name: str, email: str, workflow_name: str = "user-signup"
) -> Dict[str, Any]:
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=user_data) as response:
            if response.status == 200:
                ...
                print(
                    f"Workflow {workflow_name} submitted asynchronously. Invocation ID: {invocation_id}"
                )
...
async def query_workflow_status(
    workflow_name: str, id: str, handler_name: str = "get_status"
) -> Dict[str, Any]:
    """
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url) as response:
            if response.status == 200:
                ...
                error_text = await response.text()
                print(
                    f"Error querying workflow status: {response.status} - {error_text}"
                )
...
async def main():
    ...
    await submit_workflow(
        "user-124", "Jane Smith", "jane@mail.com", "signup-with-activities"
    )

    print("\n=== Testing Signup with Queries ===")
    await submit_workflow_async(
        "user-125", "Bob Wilson", "bob@mail.com", "signup-with-queries"
    )
    ...
    await submit_workflow_async(
        "user-126", "Alice Johnson", "alice@mail.com", "signup-with-events"
    )
    await asyncio.sleep(1)  # Give workflow time to start

    print("\n=== Testing Signup with Signals ===")
    await submit_workflow_async(
        "user-127", "Charlie Brown", "charlie@mail.com", "signup-with-signals"
    )
    await asyncio.sleep(1)  # Give workflow time to start
    ...
    await submit_workflow(
        "user-128", "Diana Prince", "diana@mail.com", "signup-with-retries"
    )
