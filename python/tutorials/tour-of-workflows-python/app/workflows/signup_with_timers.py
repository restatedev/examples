...
from app.utils import User, send_verification_email, send_reminder_email, VerifyEmailRequest
...
async def run(ctx: WorkflowContext, user: User) -> bool:
    ...
    secret = str(ctx.uuid())
    await ctx.run_typed("verify", send_verification_email, user_id=user_id, user=user, verification_secret=secret)
