...
def terminal_error_on_alice(name: str, action: str):
    if name == "Alice":
        print(f"[👻 SIMULATED] Failed to {action} for {name}: not available in this country")
        raise TerminalError(f"[👻 SIMULATED] Failed to {action} for {name}: not available in this country")
...
def send_verification_email(user_id: str, user: User, verification_secret: str):
    ...
    print(f"For the signals section, verify via: curl localhost:8080/signup-with-signals/{user_id}/verify_email --json '{{\"secret\": \"{verification_secret}\"}}'")
    print(f"For the timers section, verify via: curl localhost:8080/signup-with-timers/{user_id}/verify_email --json '{{\"secret\": \"{verification_secret}\"}}'")
...
async def send_welcome_handler(ctx: restate.Context, user: User) -> EmailServiceResponse:
...
async def create_user_handler(ctx: restate.Context, req: CreateUserRequest) -> bool:
    return await ctx.run_typed("create_user", create_user, user_id=req.user_id, user=req.user)