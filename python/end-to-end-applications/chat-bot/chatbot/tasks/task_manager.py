...
from chatbot.utils.types import TaskOpts, RunningTask, GptTaskCommand, Action
...
async def execute_command(ctx: restate.Context,
                          session_id: str,
    ...
    try:
        if command.action == Action.CREATE:
            ...
            task_id = await start_task(ctx, session_id, TaskOpts(name=name, task_type=task_type, params=params))

            new_active_tasks = active_tasks.copy()
            new_active_tasks[name] = RunningTask(name=name, task_id=task_id, task_type=task_type, params=params)
            return {
...
async def start_task(ctx: restate.Context, session_id: str, task_opts: TaskOpts) -> str:
    ...
    ctx.service_send(task_executor.execute, {
