...
public class DriverPoolService extends DriverPoolServiceRestate.DriverPoolServiceRestateImplBase {
    ...
    public void setDriverAvailable(RestateContext ctx, DriverAvailableNotification request) throws TerminalException {
        ...
        if(nextDelivery != null){
            ...
            ctx.awakeableHandle(nextDelivery).resolve(CoreSerdes.STRING_UTF8, request.getId());
            return;
        }

        // Otherwise remember driver as available
        var availableDrivers = ctx.get(AVAILABLE_DRIVERS).orElse(new LinkedList<>());
        availableDrivers.offer(request.getId());
     ...
     */
    @Override
    public void requestDriverForDelivery(RestateContext ctx, DeliveryCallback request) throws TerminalException {
        var availableDrivers = ctx.get(AVAILABLE_DRIVERS).orElse(new LinkedList<>());
