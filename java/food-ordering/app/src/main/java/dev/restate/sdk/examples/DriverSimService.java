...
public class DriverSimService extends DriverSimServiceRestate.DriverSimServiceRestateImplBase {
    ...
    public void startDriver(RestateContext ctx, OrderProto.DriverId request) throws TerminalException {
        ...
        logger.info("Starting driver " + request.getId());
        var location = ctx.sideEffect(JacksonSerdes.of(Location.class), GeoUtils::randomLocation);
        ctx.set(CURRENT_LOCATION, location);

        //TODO send location to Kafka
        producer.sendUpdate(request.getId(), JacksonSerdes.of(Location.class).serialize(location));
        ...
        DriverServiceRestate.newClient(ctx).setDriverAvailable(
                OrderProto.DriverAvailableNotification.newBuilder().setId(request.getId()).setRegion(GeoUtils.DEMO_REGION).build()
        );
...
//        DriverSimServiceRestate.newClient(ctx).oneWay().pollForWork(request);
    ...
    public void pollForWork(RestateContext ctx, OrderProto.DriverId request) throws TerminalException {
        ...
        //TODO find a better way to have no delivery job back
        if(newDeliveryJob.getId().isEmpty()){
        ...
        // TODO the mix of Protobuf and normal classes is not nice
        ctx.set(DELIVERY_STATUS, new DeliveryStatus(newDeliveryJob, false));
    ...
    public void move(RestateContext ctx, OrderProto.DriverId request) throws TerminalException {
        ...
        var nextDestination = Location.fromProto(deliveryStatus.pickedUp ? deliveryStatus.delivery.getCustomerLocation() : deliveryStatus.delivery.getRestaurantLocation());

        // Move to the next location
        var newLocation = GeoUtils.moveToDestination(currentLocation, nextDestination);
        ctx.set(CURRENT_LOCATION, newLocation);
        //TODO send location ping
        producer.sendUpdate(request.getId(), JacksonSerdes.of(Location.class).serialize(newLocation));
        ...
        if(newLocation.equals(nextDestination)){
            ...
            if(deliveryStatus.pickedUp){
                ...
                DriverServiceRestate.newClient(ctx).notifyDeliveryDelivered(request);
            ...
            DriverServiceRestate.newClient(ctx).notifyDeliveryPickup(request);
        }
