...
public class Account {
  ...
  public void deposit(Long amountCents) {
    ...
    long balanceCents = Restate.state().get(BALANCE).orElse(initializeRandomAmount());
    Restate.state().set(BALANCE, balanceCents + amountCents);
  }

  @Handler
  public Result withdraw(Long amountCents) {
    if (amountCents <= 0) {
      throw new TerminalException("Amount must be greater than 0");
    }
