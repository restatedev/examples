...
```
┌──────────┐     ┌─────────────┐     ┌─────────────────┐
│  Client  │────▶│   Restate   │────▶│ Greeter Service │
...
2. **Restate server** - Sends trace spans for ingress and individual RPCs
3. **Greeter Service** - Extracts trace context from attempt headers, creates child spans
...
### 4. Register the service with Restate
