...
import {callActivateUserAPI, emailService, sendWelcomeEmail, userService} from "../utils";
...
export const signupWithActivities = restate.workflow({
  ...
  handlers: {
    run: async (
      ...
      const success = await ctx.serviceClient(userService).createUser({id: ctx.key, ...user});
