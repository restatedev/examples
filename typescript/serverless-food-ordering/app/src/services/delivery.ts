...
async function start(ctx: restate.RpcContext, deliveryId: string, { order, promise }: OrderAndPromise) {
  ...
  // temporary placeholder: ramdom location
...
export async function createDelivery(ctx: restate.RpcContext, order: Order): Promise<void> {
    const deliveryId = await ctx.sideEffect(async () => uuidv4());
    const promise = ctx.awakeable<void>();
    await ctx.rpc(service).start(deliveryId, {order, promise: promise.id})
