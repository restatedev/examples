...
if (!(SLACK_BOT_USER_ID && SLACK_BOT_TOKEN && SLACK_SIGNING_SECRET)) {
  console.error(
    "Missing some SlackBot env variables (SLACK_BOT_USER_ID, SLACK_BOT_TOKEN, SLACK_SIGNING_SECRET)",
  );
  process.exit(1);
}
  ...
  handlers: {
    ...
    process: async (ctx: restate.Context, msg: slack.SlackMessage) => {
      ...
      const newMessage = await ctx
        .objectClient(eventDeduperSvc, channel)
  ...
  handlers: {
    isNewMessage: async (ctx: restate.ObjectContext, eventId: string) => {
      ...
      if (!known) {
        ...
        ctx
          .objectSendClient(eventDeduperSvc, ctx.key)
