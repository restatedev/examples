...
class MyService {
    ...
    suspend fun run() {
        ...
        val result = toVirtualObject<SubscriptionService>("my-sub-123")
            .request { create("my-request") }
            .call()
            .await()
        ...
        toVirtualObject<SubscriptionService>("my-sub-123")
            .request { create("my-request") }
            .send()
        ...
        awakeableHandle(awakeable.id).resolve("hello")
        ...
        runBlock { chargeBankAccount(paymentDeduplicationID, 100) }
    }
}
