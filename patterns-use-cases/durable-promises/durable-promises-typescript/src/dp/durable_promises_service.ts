  ...
  return {
    ...
    resolve: async (value?: T) => {
      const result = (await ctx
        ...
        .resolve(promiseId, value)) as ValueOrError<T>;
      return resultToPromise(result);
    },
    reject: async (errorMsg: string) => {
      const result = (await ctx
        .rpc(durablePromisesApi)
        .reject(promiseId, errorMsg)) as ValueOrError<T>;
      return resultToPromise(result);
    },
...
const durablePromisesApi = { path: durablePromisePath } as restate.ServiceApi<
  typeof durablePromiseObject
>;
...
export const durablePromiseServer = restate.router({
  resolve: (
    ctx: restate.Context,
    request: { promiseName: string; value: any }
  ...
  reject: (
    ctx: restate.Context,
    request: { promiseName: string; errorMessage: string }
  ): Promise<ValueOrError<any>> => {
    const name = ensureName(request?.promiseName);
