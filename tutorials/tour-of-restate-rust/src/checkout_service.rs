...
impl CheckoutService for CheckoutServiceImpl {
    async fn handle(
        ...
        let success = ctx.run(|| pay_client.call(&*idempotency_key, total_price)).await?;

        let email_client = EmailClient::new();
        if success {
            ctx.run(|| email_client.notify_user_of_payment_success(&*user_id)).await?;
        } else {
            ctx.run(|| email_client.notify_user_of_payment_failure(&*user_id)).await?;
        }
        Ok(success)
    }
}